...
export function ShareDialog({ thoughtmark, trigger }: ShareDialogProps) {
  ...
  const handleCopyToClipboard = async () => {
    try {
      await socialSharingManager.shareToClipboard(thoughtmark, shareOptions);
      toast({
  ...
  const handleWebShare = async () => {
    try {
      await socialSharingManager.shareViaWebShare(thoughtmark, shareOptions);
      setIsOpen(false);
    } catch (error) {
  ...
  const handleXShare = () => {
    socialSharingManager.shareToX(thoughtmark, shareOptions);
    setIsOpen(false);
  };

  const handleLinkedInShare = () => {
    socialSharingManager.shareToLinkedIn(thoughtmark, shareOptions);
    setIsOpen(false);
  };

  const handleFacebookShare = () => {
    socialSharingManager.shareToFacebook(thoughtmark);
    setIsOpen(false);
  };

  const handleInstagramShare = () => {
    socialSharingManager.shareToInstagram(thoughtmark, shareOptions);
    setIsOpen(false);
  };
  ...
  const handleInstagramStoriesShare = () => {
    socialSharingManager.shareToInstagramStories(thoughtmark, shareOptions);
    setIsOpen(false);
  };
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button
                ...
                variant="outline"
                className="justify-start text-sm bg-gradient-to-r from-purple-600/10 to-pink-600/10 border-purple-600/20 hover:bg-purple-600/20"
              >
                <MessageCircle className="w-4 h-4 mr-2" />
                IG Stories
              </Button>
